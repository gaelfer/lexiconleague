...
export default function TeacherPortalHomePage() {
  return (
    <Suspense fallback={<TeacherPortalLoader />}>
      <TeacherPortalHomeInner />
    </Suspense>
  );
}

function TeacherPortalHomeInner() {
  ...
  // No redirect when unauthenticated — show landing page with curriculum/tools, then login

  // Redirect to onboarding if not completed (teachers log in normally, then complete setup)
  useEffect(() => {
      ...
      <main className={`min-h-screen px-6 py-10 ${light ? "bg-[#F8FAFC]" : "bg-[#0B1220]"}`}>
        <div className="max-w-4xl mx-auto">
          ...
          <section className={`rounded-2xl border-2 p-8 ${light ? "border-[#3B82F6]/30 bg-[#EFF6FF]/50" : "border-[#3B82F6]/30 bg-[#1E3A5F]/20"}`}>
            ...
            <p className={`text-sm mt-2 ${textMuted}`}>
              Sign up or sign in with your school email. Quick setup — choose your grade and subject, then start hosting.
            </p>
