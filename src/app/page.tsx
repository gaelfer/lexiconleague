        ...
        <section className="relative z-10 px-4 sm:px-6 py-6 max-w-4xl mx-auto w-full">
          ...
          <div className={`flex rounded-xl p-5 ${cardBg} border ${cardBorder} gap-6`}>
            {[
              ...
              <div key={stat.label} className="flex-1 flex flex-col items-center justify-center min-w-0">
                <stat.Icon className="w-5 h-5 mb-2 shrink-0" color={stat.color} />
                <p className={`text-lg font-bold ${text}`}>{profile ? stat.format(stat.value) : "—"}</p>
                <p className={`text-xs font-medium ${textFaint}`}>{stat.label}</p>
              </div>
            ))}
          </div>
        </section>
