...
function TeacherSignupPageInner() {
  ...
  function handleEmailSignup(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { username, account_type: "teacher" },
          emailRedirectTo,
        },
      });
      if (error) {
  ...
  if (success) {
    return (
      <main className={`min-h-screen flex items-center justify-center px-6 ${light ? "bg-[#FAFAF9]" : ""}`} style={!light ? { background: SURFACE } : undefined}>
        <div className="text-center max-w-sm space-y-5">
          <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto" style={{ background: `${AMBER}20`, border: `2px solid ${AMBER}` }}>
            <svg viewBox="0 0 24 24" className="w-10 h-10" fill="none" stroke={AMBER} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </div>
          <h2 className={`text-2xl font-extrabold font-display ${light ? "text-[#0F172A]" : "text-white"}`} style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>Check your inbox!</h2>
          <p className={`text-sm font-medium ${light ? "text-[#64748B]" : "text-[#94A3B8]"}`}>{success}</p>
          <Link
            href="/auth/teacher-login"
            className="inline-block px-6 py-3 rounded-xl font-bold text-slate-900 text-sm transition-all hover:shadow-lg"
        ...
        <div className="relative z-10 text-center px-12 space-y-8">
          ...
          <div>
            ...
            <p className="text-lg mt-4 font-semibold" style={{ color: "rgba(255,255,255,0.9)" }}>
              Create your account. You&apos;ll choose your school and teaching context next.
            </p>
          </div>
          <div className="flex flex-col gap-3 text-left">
            {[
              { text: "Search & select your school", color: "rgba(255,255,255,0.9)" },
              { text: "Homeschool or public/private", color: "rgba(255,255,255,0.85)" },
              { text: "Grade level & subject", color: "rgba(255,255,255,0.8)" },
            ].map((item) => (
              <div
                key={item.text}
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className={`text-sm font-medium ${textMutedCls}`}>
              After creating your account, you&apos;ll complete setup: choose your school (or homeschool), grade level, and subject.
            </p>
            <p className={`text-sm font-medium mt-2 ${textMutedCls}`}>
              <Link href="/auth/teacher-login" className="font-bold hover:underline" style={{ color: AMBER }}>
          ...
          <form onSubmit={handleEmailSignup} className="space-y-4">
            <p className={`text-xs ${textMutedCls} mb-2`}>
              Student? <Link href="/auth/signup" className="font-semibold hover:underline" style={{ color: AMBER }}>Sign up as student</Link>
            </p>

            <div className="space-y-1.5">
              <label className={`block text-sm font-bold ${textCls}`}>Username <span className={`font-semibold ${textMutedCls}`}>(display name)</span></label>
              <input
                type="text"
                required
                maxLength={20}
            ...
            <div className="space-y-1.5">
              <label className={`block text-sm font-bold ${textCls}`}>School email</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div className="space-y-1.5">
              <label className={`block text-sm font-bold ${textCls}`}>Confirm password</label>
              <input
                type="password"
                required
                value={confirm}
