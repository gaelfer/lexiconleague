...
export function convertAvatarTexture(scene:Phaser.Scene,key:string,bodyColor?:number,outputKey=key,cropFraction=0.68){
  ...
  // Retain fine expression strokes from the actual selected SVG, never invent a mouth.
  image.data.set(pixelizeAvatar(image.data,canvas.width,canvas.height,bodyColor,key.endsWith('-eyes')?40:112));
...
export function convertAvatarGroup(scene:Phaser.Scene,layers:{key:string;color?:number}[]){
  ...
  for(const {key,color} of layers)convertAvatarTexture(scene,key,color,key,Math.min(1,extent*2));
}
