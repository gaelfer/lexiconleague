...
export function buildInnExterior(scene:Phaser.Scene,x:number,y:number,obstacle:(x:number,y:number,w:number,h:number)=>void){
  ...
  let brush=g;
  const r=(x:number,y:number,w:number,h:number,c:number)=>brush.fillStyle(c).fillRect(x,y,w,h);
  ...
  foreground(scene,x-136,bottom-240,272,88,roofArt=>{
  roofArt.scaleCanvas(2,2).translateCanvas(68,120);brush=roofArt;
