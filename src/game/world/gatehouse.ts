...
export function drawGatehouse(scene:Phaser.Scene,x:number,y:number,obstacle:(x:number,y:number,w:number,h:number)=>void){
  ...
  let brush=g;
  const r=(a:number,b:number,w:number,h:number,c:number)=>brush.fillStyle(c).fillRect(a,b,w,h);
  ...
  foreground(scene,x-80,y-146,160,58,roofArt=>{
  roofArt.scaleCanvas(2,2).translateCanvas(40,73);brush=roofArt;
