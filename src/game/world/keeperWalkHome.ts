...
export function keeperWalkHome(scene:Phaser.Scene,actor:Phaser.GameObjects.Container,walls:Phaser.Physics.Arcade.StaticGroup,destination:{x:number;y:number}){
  const snap=(v:number)=>Math.round((v-16)/32)*32+16;
  const free=(x:number,y:number)=>x>=112&&x<=1488&&y>=112&&y<=1104&&!walls.getChildren().some(object=>{
    const b=(object as Phaser.Physics.Arcade.Image).body;
    return b&&x+10>b.x&&x-10<b.right&&y+8>b.y&&y-8<b.bottom;
  });
  ...
  const step=()=>{
    ...
    const p=route[index++];if(!p){scene.tweens.add({targets:actor,y:actor.y-24,alpha:0,duration:450,onComplete:()=>actor.destroy()});return;}
