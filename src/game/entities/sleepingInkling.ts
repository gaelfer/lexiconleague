...
export function sleepingInkling(scene:Phaser.Scene,x:number,y:number,color:number,palette:InteriorPalette,baseKey='player-base'){
  ...
  const cover=scene.add.image(x,y,`interior-bed-head-${palette}`).setOrigin(0).setDisplaySize(32,32).setCrop(3,10,10,6).setDepth(21);
