...
export default function InventoryScreen({progress,bag,hearts,selectedId,onSelect,onHeal,onError}:{progress:StoryProgress;bag:StoryInventory;hearts:number;selectedId:string;onSelect:(id:string)=>void;onHeal:()=>void;onError:()=>void}){
 ...
 return <>
  ...
  <div className={s.packLayout}><section aria-label="Items section"><div className={s.inventoryBar}><h3 className={s.inventoryHeading}>In your pack</h3><span>{supplies} / {INVENTORY_CAPACITY} supplies</span></div>
   <div className={s.inventoryGrid} aria-label={`Inventory page ${current+1}`}>{Array.from({length:16},(_,i)=>{const entry=visible[i];return entry?<button key={entry.id} className={s.inventoryCell} aria-label={`${entry.name}, ${entry.quantity}`} aria-pressed={selected?.id===entry.id} title={entry.name} onClick={()=>onSelect(entry.id)}><ItemGlyph icon={entry.icon} size={36}/><span>{entry.name}</span><b>{entry.kind==='key'?'KEY':`×${entry.quantity}`}</b></button>:<div key={`empty-${i}`} className={`${s.inventoryCell} ${s.inventoryEmpty}`} aria-label={`Empty slot ${current*16+i+1}`}>·</div>;})}</div>
  ...
  {selected&&<article className={s.itemDetails}><ItemGlyph icon={selected.icon} size={40}/><div><h3>{selected.name}</h3><p>{selected.kind==='gear'?(selected.id==='sword'?'Slash with your assigned key. Learn Spin attack in Skills to charge and release a sweeping attack.':'Fire arrows with your assigned key. '+(progress.wordwoodExpedition?.gallery?'Rainstring arrows deal double damage in Wordwood.':'Arrows do not need replenishing.')):selected.description}</p>{selected.kind==='gear'&&<><p>{assigned?`Equipped to ${assigned}`:'In your pack'}</p><button className={s.stoneButton} onClick={()=>{if(!setGearEquipped(selected.id,!assigned))onError();}}>{assigned?'Unequip':'Equip'} {selected.id}</button></>}{selected.id==='herbal-reserve'&&<button className={s.stoneButton} disabled={hearts>=3||hearts<=0} onClick={onHeal}>{hearts>=3?'Health is full':'Use one bundle'}</button>}</div></article>}
 </div></>;
}
