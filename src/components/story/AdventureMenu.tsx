...
import {inventoryEntries,useHerbalReserve} from '@/lib/story/inventory';
import {TOOL_KEYS,toolSlots} from '@/lib/story/equipment';
import {learnedSkills} from '@/lib/story/skills';
...
type ObservedScene=Phaser.Scene & {player?:{x:number;y:number;hearts:number;isDying?:boolean;healFully:()=>void}};
type Position={x:number;y:number;outside:boolean;map?:{x:number;y:number}};
const PAGES=['Inventory','Map','Skills','System'] as const;
...
export default function AdventureMenu({game,chapterId,blocked,hearts}:{game:React.RefObject<Phaser.Game|null>;chapterId:number;blocked:boolean;hearts:number}){
 ...
 useEffect(()=>{
  const update=()=>{
   ...
   if(scene?.player){
    ...
    setPosition({x:scene.player.x,y:scene.player.y,outside,map});
   }
  };
 ...
 function heal(){
  const scene=game.current?.scene.getScenes(true)[0] as ObservedScene|undefined;
  if(!scene?.player||scene.player.isDying||scene.player.hearts<=0||scene.player.hearts>=3)return;
  if(useHerbalReserve()){scene.player.healFully();setStatus('Herbal bundle used · health restored');}
 }
   ...
   <div ref={panel} data-page={tab} className={`${s.gameMenu} ${settings.reducedMotion?'':s.menuMotion}`} role="dialog" aria-modal="true" aria-label="Adventure menu" tabIndex={-1} onKeyDown={event=>{
    ...
    <div className={s.consoleBody}>
     ...
     <main className={s.journalPage}>
      ...
      {tab==='Skills'&&<SkillsScreen progress={progress} onError={reportError}/>}
