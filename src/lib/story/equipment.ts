...
export const GEAR_IDS=['sword','bow','shield'] as const;
export type GearId=typeof GEAR_IDS[number];
export const TOOL_KEYS=['Q','R','F'] as const;
export type ToolKey=typeof TOOL_KEYS[number];
export function toolSlots(bag:Pick<StoryInventory,'equippedGear'|'toolSlots'>&Partial<Pick<StoryInventory,'unlockedWeapons'>>):Record<ToolKey,GearId|null>{
 const owned=bag.unlockedWeapons??['sword'];
 if(bag.toolSlots){const seen=new Set<string>();return Object.fromEntries(TOOL_KEYS.map(key=>{const id=bag.toolSlots?.[key];if(!id||!owned.includes(id)||!GEAR_IDS.includes(id)||seen.has(id))return [key,null];seen.add(id);return [key,id];})) as Record<ToolKey,GearId|null>;}
 const legacy=Array.isArray(bag.equippedGear)?bag.equippedGear:owned;
 return {Q:legacy.includes('sword')&&owned.includes('sword')?'sword':null,R:legacy.includes('bow')&&owned.includes('bow')?'bow':null,F:legacy.includes('shield')&&owned.includes('shield')?'shield':null};
}
export function equippedGear(bag:Pick<StoryInventory,'equippedGear'|'toolSlots'>&Partial<Pick<StoryInventory,'unlockedWeapons'>>):GearId[]{
...
export function assignTool(key:ToolKey,id:GearId|null){
 ...
 if(id&&!getStoryInventory().unlockedWeapons.includes(id))return false;
