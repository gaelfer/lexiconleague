...
describe('rescue gathering',()=>{
  it('keeps the family and travellers together on distinct walk-grid tiles',()=>{
    ...
    expect(Math.max(...points.map(p=>p.x))-Math.min(...points.map(p=>p.x))).toBeLessThanOrEqual(64);
    expect(Math.max(...points.map(p=>p.y))-Math.min(...points.map(p=>p.y))).toBeLessThanOrEqual(32);
