...
import { AVATAR_LAYER_WIDTH, AVATAR_LAYER_HEIGHT } from '../src/game/pixelAvatar';

describe('story world scale', () => {
  it('brings all scenes closer, keeping adventure areas closer than towns', () => {
    expect(sceneZoom(true)).toBe(WORLD_CAMERA_ZOOM*1.25);
    expect(sceneZoom(false)).toBe(WORLD_CAMERA_ZOOM*1.5);
  });
  it('uses 16px tiles and higher-resolution characters at unchanged display size', () => {
